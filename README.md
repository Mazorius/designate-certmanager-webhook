...
| Permission          | Used for                                   |
|---------------------|--------------------------------------------|
| `zones:list`        | Finding the zone for a challenge (by name) |
| `recordsets:create` | Publishing the DNS-01 TXT record           |
| `recordsets:list`   | Locating the TXT record on cleanup         |
| `recordsets:delete` | Removing the TXT record on cleanup         |
...
- `can_become_dns_member_in_openstack`: write access isolated to the Designate DNS service
- `can_become_reader_in_openstack`: required for listing zones and recordsets
